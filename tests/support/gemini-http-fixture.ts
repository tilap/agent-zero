...
export interface GeminiHttpFixtureHandle {
  readonly url: string;
...
// Gemini's generateContent / streamGenerateContent API GeminiProvider
// (Phase 19) implements. Responses are a queue: each request consumes
// the next one. Unlike the OpenAI fixture, an SSE response here never
// needs a terminal sentinel — Gemini has none (P44); the fixture just
// ends the response body.
export async function startGeminiHttpFixture(
  responses: readonly FixtureResponse[],
): Promise<GeminiHttpFixtureHandle> {
  let cursor = 0;
